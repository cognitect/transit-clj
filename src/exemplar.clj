...
(defn write-exemplar [file-name description & vals]
  ...
  (with-open [os (io/output-stream (str file-name ".json"))]
    (let [jsw (w/js-writer os)]
      (doseq [item vals] (w/write jsw item))))
  (with-open [os (io/output-stream (str file-name ".mp"))]
    (let [jsw (w/mp-writer os)]
      (doseq [item vals] (w/write jsw item)))))
...
(binding [*out* (io/writer "README.md")]
  ...
  (write-exemplar "escaped-strings" "A vector of strings containing ~." (vmap #(str "~" %) small-strings))
